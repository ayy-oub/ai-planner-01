...
export const adminValidations = {
  ...
  getUserById: [param('id').isString().notEmpty()],
  ...
  unbanUser: [param('id').isString().notEmpty()],
  deleteUser: [param('id').isString().notEmpty(), query('softDelete').optional().isBoolean()],
  restoreUser: [param('id').isString().notEmpty()],
  ...
  // 📊 User statistics (added from UserStatsDto)
  getUserStats: [
  ...
  exportUsers: [
    ...
    query('filter').optional().isIn(['all', 'active', 'inactive', 'banned', 'deleted']),
    query('dateFrom').optional().isISO8601(),
    query('dateTo').optional().isISO8601()
  ],

  // 🖥 System Stats (added all from system-stats.dto.ts)
  ...
  // 💾 Backups
  createBackup: [body('type').optional().isIn(['full', 'incremental', 'users', 'planners'])],
  getBackups: [],
  restoreBackup: [param('backupId').isString().notEmpty()]
};
