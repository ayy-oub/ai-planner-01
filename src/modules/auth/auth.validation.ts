...
export const updateProfileValidation = [
  ...
  body('preferences.defaultView')
    .optional()
    .isIn(['daily', 'weekly', 'monthly', 'grid', 'list', 'calendar'])
    .withMessage('Default view must be one of: daily, weekly, monthly, grid, list, calendar'),
  ...
  body('preferences.language')
    .optional()
    .custom((value) => {
      const iso6391 = ['en', 'fr', 'es', 'ar'];
      if (!iso6391.includes(value)) throw new Error('Language must be a valid ISO 639-1 code');
      return true;
    }),
